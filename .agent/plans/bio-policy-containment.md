...
- The main checkout has unrelated in-flight edits (alpha-search work), including an
  `ALPHA_SEARCH_PIPELINE` preset in guard-pipeline.ts that still references the old `"cyberBlock"`
  step key. After merging this branch that reference must become `"policyBlock"`; TypeScript will
  catch it (the key no longer exists in `GuardStepKey`).
