...
describe("ProxyForwarder provider failure session release", () => {
  ...
  it("同步 hedge 胜出 shadow session 时保留 releaseAgent cleanup 回调", async () => {
    ...
    const copyCyberCheckAdmissionCorrelationFrom = vi.fn();
    const target = {
      ...
      copyCyberCheckAdmissionCorrelationFrom,
    } as unknown as ProxySession;
    const source = {
    ...
    expect(copyCyberCheckAdmissionCorrelationFrom).toHaveBeenCalledWith(source);
  });
});
