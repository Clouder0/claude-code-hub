...
describe("buildProviderBillingEventsQuery", () => {
  it("binds endpoint, half-open time-window, user, and provider filters as parameters", () => {
    ...
    expect(query.sql).toContain("ledger.hedge_losers @> $7::jsonb");
    expect(query.sql).toContain("ledger.hedge_losers @> $8::jsonb");
    expect(query.sql).toContain("WHERE provider_id = $17");
  ...
  it("deduplicates loser identities by first appearance and emits stable event IDs", () => {
    ...
    expect(sql).toContain("SELECT DISTINCT ON (request_id, provider_id, attempt_number)");
    expect(sql).toContain("ORDER BY request_id, provider_id, attempt_number, ordinality ASC");
  ...
  it("subtracts deduplicated settled loser costs from the winner and clamps at zero", () => {
    ...
    expect(sql).toContain(
      "SELECT request_id, COALESCE(SUM(cost_usd), 0) AS cost_usd FROM settled_losers GROUP BY request_id"
    );
